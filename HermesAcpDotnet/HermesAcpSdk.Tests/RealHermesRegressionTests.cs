...
public sealed class RealHermesRegressionTests
{
    [Fact]
    public async Task PingPrompt_ReturnsPong_WhenRealHermesRegressionIsEnabled()
    {
        if (!IntegrationTestSupport.IsOptedIn("HERMES_REAL_REGRESSION"))
        {
            return;
        }

        var runStore = IntegrationTestSupport.CreateTempDirectory("hermes-real-regression");
        var shouldKeepArtifacts = IntegrationTestSupport.IsOptedIn("HERMES_KEEP_ARTIFACTS");
        var passed = false;

        try
        {
            var result = await RunAgainstRealHermesAsync(
                runStore,
    ...
    [Fact]
    public async Task RepositoryAnalysis_ReturnsRepoSpecificPaths_WhenRealHermesRegressionIsEnabled()
    {
        if (!IntegrationTestSupport.IsOptedIn("HERMES_REAL_REGRESSION"))
        {
            return;
    ...
    private static async Task<HermesRunResult> RunAgainstRealHermesAsync(
        string runStore,
        string prompt,
        int timeoutSeconds,
        CancellationToken cancellationToken)
    {
        ...
        var executable = Environment.GetEnvironmentVariable("HERMES_REAL_EXECUTABLE") ?? "hermes";
        var args = (Environment.GetEnvironmentVariable("HERMES_REAL_ARGS") ?? "acp")
            .Split(' ', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);

        Assert.True(IntegrationTestSupport.CanRun(executable, "--help"), $"Unable to launch Hermes executable '{executable}'.");

        var options = new HermesAcpOptions
        {
            ActiveProfile = "real-hermes",
            Profiles = new Dictionary<string, HermesLaunchProfile>(StringComparer.OrdinalIgnoreCase)
            {
                ["real-hermes"] = new HermesLaunchProfile
                {
                    ...
                    Authentication = new HermesAuthenticationOptions(),
                    SessionDefaults = new HermesSessionDefaults
                    {
                        ModeId = "analysis",
        ...
        return await new HermesValidationRunner(options).RunAsync(
            new HermesRunRequest
            {
                ProfileName = "real-hermes",
                Prompt = prompt,
            },
            cancellationToken);
    }

    private static void AssertSuccessfulRun(HermesRunResult result, string runStore)
    {
        Assert.Equal("real-hermes", result.ProfileName);
