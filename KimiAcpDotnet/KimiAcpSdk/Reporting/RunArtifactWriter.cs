...
{
    private static readonly JsonSerializerOptions ArtifactJsonOptions = new(KimiAcpSdk.Protocol.KimiJson.Default)
    {
        WriteIndented = true,
    };
    ...
    private sealed record FailureDiagnostics(string? Stage, string? Message, bool HasStandardError);

    public async Task<KimiRunArtifact> WriteAsync(KimiRunResult result, CancellationToken cancellationToken = default)
    {
        ...
        if (!string.IsNullOrWhiteSpace(result.FailureStage) || !string.IsNullOrWhiteSpace(result.FailureMessage) || standardErrorPath is not null)
        {
            diagnosticsPath = Path.Combine(runDirectory, "diagnostics.json");
            var diagnostics = new FailureDiagnostics(result.FailureStage, result.FailureMessage, standardErrorPath is not null);
            await File.WriteAllTextAsync(diagnosticsPath, JsonSerializer.Serialize(diagnostics, ArtifactJsonOptions), cancellationToken);
        }

        return new KimiRunArtifact(runDirectory, summaryPath, reportPath, transcriptLogPath, transcriptJsonPath, standardErrorPath, diagnosticsPath);
    }
