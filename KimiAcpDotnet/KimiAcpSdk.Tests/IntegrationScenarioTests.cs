...
public sealed class IntegrationScenarioTests
{
    ...
    {
        if (!IntegrationTestSupport.IsOptedIn("KIMI_INTEGRATION"))
        {
            return;
        }
        ...
        var tempArtifacts = IntegrationTestSupport.CreateTempDirectory("kimi-live-runs");
        ...
        try
        {
            var options = new KimiAcpOptions
            {
                ...
                {
                    ["integration"] = new KimiLaunchProfile
                    {
            ...
            var result = await new KimiValidationRunner(options).RunAsync(new KimiRunRequest
            {
    ...
    {
        var configuredExecutable = Environment.GetEnvironmentVariable("KIMI_INTEGRATION_EXECUTABLE");
        if (!string.IsNullOrWhiteSpace(configuredExecutable))
        {
            var args = (Environment.GetEnvironmentVariable("KIMI_INTEGRATION_ARGS") ?? string.Empty)
        ...
        var scriptPath = Path.Combine(AppContext.BaseDirectory, "Fixtures", "fake_kimi_adapter.py");
